...
def find_rpm(min_rpm, max_rpm, maxspeed, downshift_dir, upshift_dir):
    ...
    no_of_gears = np.size(upshift_dir + 1)

    # For Test
    # no_of_gears = 4

    # For importing the gear lines from the excel input
    for i in range(1, np.size(upshift_dir)):
        if i == 1:
            gears[str(i) + " up"] = upshift_dir[i+1]
            gears[str(i) + " down"] = [[0, 0], [0, 100]]
        elif i == np.size(upshift_dir):
            gears[str(i) + " up"] = [[maxspeed, 0], [maxspeed, 100]]
            gears[str(i) + " down"] = downshift_dir[i]
        else:
            gears[str(i) + " up"] = upshift_dir[i+1]
            gears[str(i) + " down"] = downshift_dir[i]
